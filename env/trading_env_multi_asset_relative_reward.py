...
class TradingEnv(gym.Env):
    """
    Entorn multi-actiu amb reward relativa.
    ...
    La recompensa no és només el retorn absolut de l'actiu seleccionat,
    sinó el retorn relatiu respecte a la mitjana dels tres actius.

    reward = selected_return - mean_market_return
             - switching_cost
             - inactivity_cost
    """
    ...
    def __init__(
        ...
        obs_dim = len(feature_columns) + 2  # features + current_asset + holding_duration
    ...
    def step(self, action: int):
        ...
        btc_return = row[self.return_columns[0]]
        gld_return = row[self.return_columns[1]]
        uso_return = row[self.return_columns[2]]

        all_returns = np.array([btc_return, gld_return, uso_return], dtype=np.float64)
        market_mean_return = float(np.mean(all_returns))

        selected_return = row[self.return_columns[new_asset]]
        selected_volatility = row[self.vol_columns[new_asset]]

        # Reward relativa: premiar fer-ho millor que la mitjana dels actius disponibles
        reward = selected_return - market_mean_return
        ...
        if new_asset != previous_asset:
            ...
            inactivity_cost = self.inactivity_penalty * np.log1p(self.holding_duration)
        ...
        info = {
            ...
            "relative_return": selected_return - market_mean_return,
            "selected_volatility": selected_volatility,
            "switching_cost": switching_cost,
            "inactivity_cost": inactivity_cost,
            "holding_duration": self.holding_duration,
            "btc_target_ret": btc_return,
            "gld_target_ret": gld_return,
            "uso_target_ret": uso_return,
        }
