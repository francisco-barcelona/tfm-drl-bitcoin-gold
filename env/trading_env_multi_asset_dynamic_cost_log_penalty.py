...
class TradingEnv(gym.Env):
    ...
    - assignació dinàmica entre BTC, GLD i USO
    - cost de canvi d'actiu dinàmic segons volatilitat
    - penalització d'inactivitat suavitzada amb log
    ...
    def step(self, action: int):
        ...
        if new_asset != previous_asset:
            ...
            # Penalització suavitzada:
            # creix amb la durada, però més lentament que una penalització lineal.
            inactivity_cost = (
                self.inactivity_penalty
                * np.log1p(self.holding_duration)
            )
